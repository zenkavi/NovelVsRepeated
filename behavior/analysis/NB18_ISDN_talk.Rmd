...
```{r}
source(paste0(helpers_path, '04_stim_closeness_helpers.R'))
```

```{r}
data_yn_cht_wshape = get_cht_labels_for_all_subs(data = data_yn_clean, w_shape = TRUE)
```

```{r}
with(data_yn_cht_wshape, table(subnum, type_chr, time_bin))
```

```{r warning=FALSE}
data_yn_cht_wshape%>%
  mutate(time_bin = factor(time_bin, levels = c("Week 3", "Week 2", "Week 1", "Day 3", "Day 2", "Day 1"))) %>%
  mutate(type_chr = ifelse(type_chr == "CHT", "Close", type_chr),
         type_chr = factor(type_chr, levels = c("Over Exposure", "Close", "Regular Exposure"))) %>%
  ggplot() +
  geom_hline(aes(yintercept =.5), color="gray")+
  geom_vline(aes(xintercept = 0), color="gray")+
  geom_line(aes(val_diff, yesChosen, color = time_bin, alpha = time_bin), stat="smooth", method = "glm", method.args = list(family = "binomial"), formula = "y ~ x", se = FALSE, size = 1.5)+
...
types = unique(data_yn_cht_wshape$type_chr)
normMax = 1
normMinDay = 0
normMinVal = -1
...
for(cur_sub in subnums){
  for(cur_type in types){
    ...
    cur_dat = data_yn_cht_wshape %>%
      ungroup() %>%
      filter(subnum == cur_sub & type_chr == cur_type) %>%
      mutate(time_bin = as.factor(time_bin)) %>%
      group_by(day) %>% # Grouping by day to normalize valdiff matters. This makes sure every day ranges [-1,1]
      mutate(norm_valDiff = (normMax - normMinVal) / (max(val_diff) - min(val_diff)) * (val_diff - max(val_diff)) + (normMax)) %>%
      ungroup() %>%
    ...
    # If type is not CHT down sample to make the amount of data comparable
    if(cur_type != "CHT"){
      tb1 =  cur_dat %>% filter(time_bin == "Day 1") %>% sample_n(50)
      tb2 =  cur_dat %>% filter(time_bin == "Day 2") %>% sample_n(50)
      tb3 =  cur_dat %>% filter(time_bin == "Day 3") %>% sample_n(50)
      tb4 =  cur_dat %>% filter(time_bin == "Week 2") %>% sample_n(180)
      tb5 =  cur_dat %>% filter(time_bin == "Week 3") %>% sample_n(180)
      
      cur_dat = rbind(tb1, tb2, tb3, tb4, tb5)
    }
    
    contrasts(cur_dat$time_bin) = my_helmert
    
    cur_m <- brm(formula = yesChosen ~ norm_valDiff * time_bin,
    # cur_m <- brm(formula = yesChosen ~ norm_valDiff,
                 data= cur_dat,
                 family = bernoulli(link = "logit"),
                 warmup = 500,
                 iter = 2500,
...
yn_par_summary = as_tibble(yn_logit_draws) %>%
  ...
  mutate(key=recode(key, 
         ...
         key = factor(key, levels = c("Intercept", "Value Diff [-1, 1]", "Day 1 vs Later", "Day 2 vs Later", "Day 3 vs Later", "Week 2 vs Later", "Value Diff x Day 1 vs Later", "Value Diff x Day 2 vs Later", "Value Diff x Day 3 vs Later", "Value Diff x Week 2 vs Later"))
         # key = factor(key, levels = c("Intercept", "Value Diff [-1, 1]"))
         ) %>%
  group_by(subnum, type, key) %>%
  summarise(.groups = 'keep',
            m = mean(value),
...
yn_par_summary %>%
  ...
  # mutate(type_chr = ifelse(type == 0, "Regular Exposure", "Over Exposure")) %>%
  filter(key %in% c("Intercept", "Value Diff [-1, 1]", "Value Diff x Day 1 vs Later")) %>%
    # filter(key %in% c("Value Diff [-1, 1]")) %>%
  ggplot(aes(m, subnum, color = type)) +
  geom_vline(aes(xintercept = 0), color = "gray") + 
  geom_point(position = position_dodge(width=.75))+
  geom_errorbarh(aes(xmin = l95, xmax = h95, height=.2), position = position_dodge(width=.75))+
  facet_wrap(~key, scales = 'free_x',labeller = labeller(key = label_wrap_gen(10))) +
...
What should a continuous measure of closeness to HT look like?
Assumption: color and shape are more salient than orientation.
But what about how consequential an attribute is for the stim reward? Eg. shape is more consequential than orientation 
And also continuity of levels? For orientation and filling one levels gives info on other levels but for shape they're not ordered

What plot/conclusion do I want to show?
The closer a stim is to an HT, the better the behavior? The better or the more HT like?

What is the interesting questions here?
Are you learning about the HT in an isolated manner? Or is what you have learned about them leaking to other stims in some way? 
  - This is sort of a straw man argument. If there was no generalization there would be no reason for behavior to RE to improve. So there is learning for more than just the stim value but what is the representation that is being updated?
I.e. evidence for some generalization
In other words maybe: are you learning about the stimuli directly or values associated with the attribute levels?

I don't think I've been thinking about this the right way. Of course there is some 

```{r}
data_yn_clean %>%
  group_by(subnum, day, yesChosen, type) %>%
  summarise(.groups = "keep",
            median_rt = median(rt)) %>%
  arrange(subnum, day, type)
```
