...
Takes ~ 18 seconds with these parameters for 800 trials.

```{r}
# start = Sys.time()
sim_data = sim_task(stims, d = .005, sigma = .005, alpha = .3, theta = .2, nonDecisionTime = 300, bias = 0, barrierDecay = 0, return_values = TRUE)
# end = Sys.time()
...
```{r}
summary(sim_data$out$reactionTime)
```

```{r}
sim_data$out %>%
  group_by(type) %>%
  summarise(median_rt = median(reactionTime))
```
