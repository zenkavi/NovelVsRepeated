...
Source `sim_yn_ddm.R` and `yn_ddm.R`. See `NB12_DDM_statespace_functions.Rmd` for details on how to use these functions. Command will look something like:

```
sim_task(stimuli = sub_stims, model_name = "model1", sim_trial_list_ = sim_trial_list, d = cur_d, sigma = cur_sigma, nonDecisionTime = cur_nonDecisionTime, bias = cur_bias, barrierDecay = cur_barrierDecay, maxIter = 300, debug = T)
```

where sub_stims should be the trials for each day and type
