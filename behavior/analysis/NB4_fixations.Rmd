...
p = data_bc_fix %>%
  ...
  summarise(sem_choice_firstFix = sd(mean_choice_firstFix)/sqrt(n()),
            mean_choice_firstFix = mean(mean_choice_firstFix), .groups = 'keep') %>%
  mutate(day = paste0("Day ", day),
         day = factor(day, levels = c("Day 3", "Day 7", "Day 11"))) %>%
  ggplot(aes(firstFixDurBin, mean_choice_firstFix, color = type))+
  geom_point()+
  geom_errorbar(aes(ymin = mean_choice_firstFix - sem_choice_firstFix, ymax = mean_choice_firstFix + sem_choice_firstFix), width = 0)+
  geom_line(aes(group = type))+
...
ggsave(file=paste0(fig_out_path, 'bc_group_pFirstFixatedOverFirstFixDurByStim.jpg'), p, height = 3, width=9, units="in")
p
```
...
p = data_bc_fix %>%
  ...
  mutate(firstFixChosen = ifelse(leftFix == leftChosen, 1, 0),
         ...
         firstFixDurBin = factor(firstFixDurBin, levels = c("<100", "100-200", "200-300", "300-400", "400-500", ">500"))) %>%
  group_by(subnum, day, firstFixDurBin, type) %>%
  summarise(sem_choice_firstFix = sd(firstFixChosen)/sqrt(n()),
            mean_choice_firstFix = mean(firstFixChosen), .groups = 'keep') %>%
  mutate(day = paste0("Day ", day),
         day = factor(day, levels = c("Day 3", "Day 7", "Day 11"))) %>%
  ggplot(aes(firstFixDurBin, mean_choice_firstFix, color = type))+
  geom_point()+
...
ggsave(file=paste0(fig_out_path, 'bc_pFirstFixatedOverFirstFixDurByStim.jpg'), p, height = 5, width=8, units="in")
p
```
