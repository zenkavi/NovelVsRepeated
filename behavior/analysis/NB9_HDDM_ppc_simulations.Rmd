---
title: "Novel vs. repeated choice project: Y/N Task HDDM posterior predictive checks"
output: 
  html_document:
    toc: true
    toc_float: true
...
fn_draws = paste0(helpers_path, '/inputs/sim_sub_yn_logit_draws.csv')
fn_ppc = paste0(helpers_path, '/inputs/sim_sub_yn_logit_ppc.csv')
...
```{r}
as.data.frame(sim_yn_logit_draws) %>%
...
```{r}
p = ggplot() +
  geom_point(data = sim_group_summary, aes(abs_val_diff_bin_str, mean_y, color = type, alpha = time_bin, size = time_bin))+
  geom_line(data = sim_group_summary, aes(abs_val_diff_bin_str, mean_y, color = type, group = time_bin, alpha = time_bin)) +
  facet_wrap(~type)+
...
ggplot() +
  geom_point(data = sim_sub_summary, aes(abs_val_diff_bin_str, mean_y, color = type, shape = time_bin), size = 2.5)+
  geom_line(data = sim_sub_summary, aes(abs_val_diff_bin_str, mean_y, color = type, group = time_bin, linetype = time_bin)) +
