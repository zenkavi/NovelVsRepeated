...
`
```{r}
# imgs # rolling preview
i = 10
imgs[i]
```

```{r}
# tmp = make_stim_img(shape = ht_features$shape[i], orientation = ht_features$orientation[i]+15, filling = ht_features$filling[i])
tmp_img = make_stim_img(shape = ht_features$shape[i], orientation = ht_features$orientation[i], filling = ht_features$filling[i]-0.2)
tmp_img
```
