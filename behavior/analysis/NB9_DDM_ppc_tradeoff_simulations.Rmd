...
```{r}
sim_post_pred_data = function(n_samples, subs, types, days, interim_save = TRUE){
  
  set.seed(20572380)
  
  if(!exists("data_yn_clean")){
    library(tidyverse)
    library(here)
  ...
  if(!exists("all_results_df")){
    fn = file.path(here(), "inputs",'yn_hddm_mcmc_draws.csv')
    
    all_results_df = read.csv(fn)
  }
  
  fn = file.path(here(), "inputs", paste0('yn_ddm_ppc_interim_save.csv'))
  for(i in 1:length(subs)){
    cur_sub = subs[i]
    print(paste0("Beginning sub-", cur_sub))
    
    for(j in 1:length(types)){
      cur_type = types[j]
      print(paste0("Beginning type: ", cur_type))
      
      for(k in 1:length(days)){
        cur_day = days[k]
        print(paste0("Beginning day: ", cur_day))  
        
        cur_pred = sim_subj_cond_day_nsamples(n_samples, cur_sub, cur_type, cur_day)
...
```{r}
# fn = file.path(here(), "inputs",'yn_ddm_ppc_data.csv')
# 
# if(file.exists(fn)){
#   yn_ddm_ppc_data = read.csv(fn)
#   
# } else{
#   
#   n_samples = 100
...
#   types = c('HT', 'RE')
#   
#   yn_ddm_ppc_data = sim_post_pred_data(n_samples, subs, types, days)
#   
#   write.csv(pred_data, fn, row.names = F)
# }
```
...
```{r}
sim_subj_cond_day_nsamples(10, 601, "HT", 5)
```

How similar are the multiple sets of simulated data to each other?

How similar is the simulated data to the true data?

```{r}
tmp %>%
  filter(rt < 3) %>%
  mutate(rt = ifelse(yesChosen == 0, rt*(-1), rt),
         pred_rt = ifelse(pred_yesChosen == 0, pred_rt*(-1), pred_rt)) %>%
  ggplot()+
  geom_density(aes(x = pred_rt), position = "identity", alpha = .5) +
  geom_histogram(aes(x = rt, y = ..density..), bins = 30,  position = "identity", alpha = .5, color = NA)+
  # facet_grid(. ~ sample)+
  theme(panel.grid = element_blank(),
        axis.text.x = element_text(margin = margin(t = 5)))+
  geom_vline(aes(xintercept = 0), color = "gray") +
