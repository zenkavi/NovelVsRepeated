...
fn_draws = paste0(helpers_path, '/inputs/yn_accuracy_draws.csv')
fn_ppc = paste0(helpers_path, '/inputs/yn_accuracy_ppc.csv')

if(file.exists(fn_draws) & file.exists(fn_ppc)){
  yn_accuracy_draws = read.csv(fn_draws)
  yn_accuracy_ppc = read.csv(fn_ppc)
} else{
  subnums = unique(data_yn_clean$subnum)
  
  yn_accuracy_draws = data_frame()
  yn_accuracy_ppc = data_frame()
  ...
  for(i in 1:length(subnums)){
    ...
    cur_yn_data = data_yn_clean %>% 
      ungroup() %>%
      filter(subnum == cur_sub) %>%
      select(abs_val_diff_std, day_std, type, abs_val_diff, day, correct)
    
    
    cur_yn_logit <- brm(formula = correct ~ abs_val_diff_std * day_std * type,
                        data= cur_yn_data,
                        family = bernoulli(link = "logit"),
                        warmup = 500,
                        iter = 2500,
    ...
    yn_accuracy_draws = rbind(yn_accuracy_draws, cur_draws)
    
    cur_pred = fitted(cur_yn_logit, newdata = cur_yn_data %>%
                        select(-abs_val_diff, -day, -correct), scale = "linear") %>%
      as_tibble() %>%
      transmute(prob = Estimate %>% inv_logit_scaled()) %>%
      bind_cols(cur_yn_data) %>%
      mutate(subnum = cur_sub)
    
    yn_accuracy_ppc = rbind(yn_accuracy_ppc, cur_pred)
  }
  names(yn_accuracy_draws) = gsub(":", ".", names(yn_accuracy_draws))
  write.csv(yn_accuracy_draws, fn_draws, row.names = F)
  write.csv(yn_accuracy_ppc, fn_ppc, row.names = F)
...
```{r}
as.data.frame(yn_accuracy_draws) %>%
...
```
```{r}
p = as.data.frame(yn_accuracy_draws) %>%
  select("b_abs_val_diff_std.day_std", "b_abs_val_diff_std.typeRE", "b_abs_val_diff_std.day_std.typeRE", subnum, .chain, .iteration) %>%
  gather(key, value, -subnum, -.chain, -.iteration) %>%
...
p
# ggsave(file=paste0(fig_out_path, 'yn_accuracy_interactionParEsts.jpg'), p, height = 3, width = 9, units="in")
```
...
p = data_yn_clean %>%
  ...
  ggplot(aes(abs_val_diff_bin_str, mean_correct, color=type))+
  geom_point(size = 2.5)+
...
# ggsave(file=paste0(fig_out_path, 'yn_group_accuracyByAbsValDiff.jpg'), p, height = 3, width=9, units="in")
p
```
...
p = yn_accuracy_ppc %>% 
  ...
  mutate(day = paste0("Day ", day),
         day = factor(day, levels = c("Day 1", "Day 3", "Day 7", "Day 11"))) %>%
  ggplot()+
  geom_line(aes(x = abs_val_diff, y = prob, color=type))+
  geom_point(data = yn_data_summary, aes(x = abs_val_diff, y = mean_correct, color = type)) +
  geom_errorbar(data = yn_data_summary, aes(x = abs_val_diff, y = mean_correct, ymin = mean_correct - sem_correct, ymax = mean_correct + sem_correct, color= type), width = 0) +
  facet_grid(subnum~day) + 
  labs(x = "|Value Stim - Value Reference|", y = "p(Correct)", title="Y/N Task - True vs Predicted", color="")+
  theme(panel.grid = element_blank(),
        legend.position = "bottom",
        axis.text.x = element_text(angle = 15, margin = margin(t = 5)),
        legend.box.margin=margin(t = -10))+
...
# ggsave(file=paste0(fig_out_path, 'yn_accuracy_ppc.jpg'), p, height = 6, width = 10, units="in")
p
```
...
```{r}
fn_draws = paste0(helpers_path, '/inputs/bc_accuracy_draws.csv')
fn_ppc = paste0(helpers_path, '/inputs/bc_accuracy_ppc.csv')

if(file.exists(fn_draws) & file.exists(fn_ppc)){
  bc_accuracy_draws = read.csv(fn_draws)
  bc_accuracy_ppc = read.csv(fn_ppc)
} else{
  subnums = unique(data_bc_clean$subnum)
  
  bc_accuracy_draws = data_frame()
  bc_accuracy_ppc = data_frame()
  ...
  for(i in 1:length(subnums)){
    ...
    cur_bc_accuracy <- brm(formula = correct ~ abs_val_diff_std * day_std * type,
                        data= cur_bc_data,
                        family = bernoulli(link = "logit"),
                        warmup = 500,
                        iter = 2500,
    ...
    cur_draws = as_draws_df(cur_bc_accuracy)
    cur_draws$subnum = cur_sub
    
    bc_accuracy_draws = rbind(bc_accuracy_draws, cur_draws)
    
    cur_pred = fitted(cur_bc_accuracy, newdata = cur_bc_data %>%
                        select(-abs_val_diff, -day, -correct), scale = "linear") %>%
      as_tibble() %>%
      transmute(prob = Estimate %>% inv_logit_scaled()) %>%
      bind_cols(cur_bc_data) %>%
    ...
    bc_accuracy_ppc = rbind(bc_accuracy_ppc, cur_pred)
  }
  names(bc_accuracy_draws) = gsub(":", ".", names(bc_accuracy_draws))
  write.csv(bc_accuracy_draws, fn_draws, row.names = F)
  write.csv(bc_accuracy_ppc, fn_ppc, row.names = F)
  
  rm(cur_bc_data, cur_bc_accuracy, cur_draws, cur_pred)
}
...
```{r}
as.data.frame(bc_accuracy_draws) %>%
  select("b_abs_val_diff_std.day_std", "b_abs_val_diff_std.typeRE", "b_abs_val_diff_std.day_std.typeRE", subnum, .chain, .iteration) %>%
  gather(key, value, -subnum, -.chain, -.iteration) %>%
  ggplot(aes(.iteration, value, color = as.factor(.chain)))+
  geom_line()+
...
```{r}
p = as.data.frame(bc_accuracy_draws) %>%
  select("b_abs_val_diff_std.day_std", "b_abs_val_diff_std.typeRE", "b_abs_val_diff_std.day_std.typeRE", subnum, .chain, .iteration) %>%
  gather(key, value, -subnum, -.chain, -.iteration) %>%
  mutate(key = ifelse(key == "b_abs_val_diff_std.day_std", "|Value Diff| * Day", ifelse(key == "b_abs_val_diff_std.typeRE", "|Value Diff| * Type", ifelse(key == "b_abs_val_diff_std.day_std.typeRE", "|Value Diff| * Day * Type", NA))),
         key = factor(key, levels = c("|Value Diff| * Type", "|Value Diff| * Day", "|Value Diff| * Day * Type"))) %>%
...
p
# ggsave(file=paste0(fig_out_path, 'bc_accuracy_interactionParEsts.jpg'), p, height = 3, width = 9, units="in")
```
...
# ggsave(file=paste0(fig_out_path, 'bc_group_accuracyByAbsValDiff.jpg'), p, height = 3, width=9, units="in")
p
```
...
p = bc_accuracy_ppc %>% 
  ...
  ggplot()+
  geom_line(aes(x = abs_val_diff, y = prob, color=type))+
  geom_point(data = bc_data_summary, aes(x = abs_val_diff, y = mean_correct, color = type)) +
  geom_errorbar(data = bc_data_summary, aes(x = abs_val_diff, y = mean_correct, ymin = mean_correct - sem_correct, ymax = mean_correct + sem_correct, color= type), width = 0) +
  facet_grid(subnum~day) + 
  labs(x = "|Value Left - Value Right|", y = "p(Correct)", title="BC Task - True vs Predicted", color="")+
  theme(panel.grid = element_blank(),
        legend.position = "bottom",
        axis.text.x = element_text(angle = 15, margin = margin(t = 5)),
        legend.box.margin=margin(t = -10))+
...
# ggsave(file=paste0(fig_out_path, 'bc_accuracy_ppc.jpg'), p, height = 6, width = 10, units="in")
p
```
