...
yn_par_summary = as.data.frame(yn_logit_draws) %>%
  select("b_val_diff_std.day_std", "b_val_diff_std.typeRE", "b_val_diff_std.day_std.typeRE", subnum,) %>%
  gather(key, value, -subnum) %>%
  mutate(key = ifelse(key == "b_val_diff_std.day_std", "Value Diff * Day", ifelse(key == "b_val_diff_std.typeRE", "Value Diff * Type (RE)", ifelse(key == "b_val_diff_std.day_std.typeRE", "Value Diff * Day * Type (RE)", NA))),
         key = factor(key, levels = c("Value Diff * Type (RE)", "Value Diff * Day", "Value Diff * Day * Type (RE)"))) %>%
  group_by(subnum, key) %>%
...
p = yn_par_summary %>%
  ...
  facet_wrap(~key) +
  facet_wrap(~key) +
  labs(x = "Parameter Estimate", y = "Subject", color = "", title = "95% HDI for posterior slopes") +
...
p = as_tibble(yn_logit_draws) %>%
  ...
  mutate(key = ifelse(key == "b_typeRE", "Type", ifelse(key == "b_val_diff_std", "Value Diff", ifelse(key == "b_val_diff_std.typeRE", "Type * Value Diff", NA))),
         key = factor(key, levels = c("Type", "Value Diff", "Type * Value Diff"))) %>%
  ...
  ggplot(aes(day, m))+
  geom_point()+
    geom_errorbar(aes(ymin = l95, ymax = h95), width=.2)+
  facet_grid(key~subnum, scales="free")+
    theme(panel.grid = element_blank(),
        axis.text.x = element_text(angle = 15, margin = margin(t = 5)))+
  labs(x="", y="Parameter Estimate", title="Y/N Task")

# ggsave(file=paste0(fig_out_path, 'yn_logit_valdiff_slope.jpg'), p, height = 6, width = 10, units="in")
p
```
...
p = as_tibble(bc_logit_draws) %>%
  ...
  facet_grid(key~subnum, scales="free")+
    theme(panel.grid = element_blank(),
        axis.text.x = element_text(angle = 15, margin = margin(t = 5)))+
  labs(x="", y="Parameter Estimate", title="BC Task")

# ggsave(file=paste0(fig_out_path, 'bc_logit_valdiff_slope.jpg'), p, height = 6, width = 10, units="in")
p
```

```{r}
tmp = as_tibble(bc_logit_draws) %>%
  filter(subnum == 601 & day == "day 1") %>%
  select(b_val_diff_std)
  

HDInterval::hdi(tmp)[1]  
```
...
bc_addm_summary %>%
  ...
  geom_point(position = position_dodge(width=.75))+
  geom_errorbar(aes(ymin = Lower95, ymax = Upper95), position = position_dodge(width=.75))+
  facet_grid(par_group ~ par_name, scale = "free_y")+
  scale_color_brewer(palette = "Dark2")+
  theme(panel.grid = element_blank(),
        legend.position = "bottom",
        axis.text.x = element_text(angle = 15, margin = margin(t = 5)),
...
bc_addm_summary %>%
  ...
  geom_errorbar(aes(ymin = Lower95, ymax = Upper95), position = position_dodge(width=.9))+
