...
# Rscript --vanilla fit_yn_sub_cb_hddm.R --type HT --sub 601
# Rscript --vanilla fit_yn_sub_cb_hddm.R --type RE --sub 619
...
fn = paste0('YN_HDDM_CB_FIT_', stim_type_str, '_', subnum_str)

data <- read.csv(file.path(input_path , "data_choiceYN.csv"))
model_fn <- file.path(code_path, "yn_hddm_cb_jags.txt")
...
data = data %>%
  filter(reference != -99) %>%
  filter(rt > .4 & rt < 5) %>% # discard very long and short RT trials
  ...
  mutate(rtPN = ifelse(yesChosen == 1, rt, (-1)*rt),
         timestep = (rt*1000)/10 ) #round this to integer?
...
dat <- dump.format(list(N=N, y=y, idxP=idxP, v_diff=v_diff, ns=ns, timestep=timestep))
...
#                             decay.alpha= .001, decay.beta= 1,  b.mu=0.2, b.pr=0.05, bias.mu=0.5, bias.kappa=1,
#                             y_pred=y, .RNG.name="base::Super-Duper", .RNG.seed=99999 ))
#
# inits2 <- dump.format(list( alpha.mu=alpha.mu2, alpha.pr=alpha.si2,
#                             decay.alpha= .005, decay.beta = 2,  b.mu=1, b.pr=0.05, bias.mu=0.6, bias.kappa=1,
#                             y_pred=y, .RNG.name="base::Wichmann-Hill", .RNG.seed=1234))
#
# inits3 <- dump.format(list( alpha.mu=alpha.mu3, alpha.pr=alpha.si3,
#                             decay.alpha= .1, decay.beta= 3,  b.mu=1.3, b.pr=0.05, bias.mu=0.4, bias.kappa=1,
#                             y_pred=y, .RNG.name="base::Mersenne-Twister", .RNG.seed=6666 ))

inits1 <- dump.format(list( alpha.mu=alpha.mu1, alpha.pr=alpha.si1,
                            b.mu=0.2, b.pr=0.05, bias.mu=0.5, bias.kappa=1,
                            y_pred=y, .RNG.name="base::Super-Duper", .RNG.seed=99999 ))

inits2 <- dump.format(list( alpha.mu=alpha.mu2, alpha.pr=alpha.si2,
                            b.mu=1, b.pr=0.05, bias.mu=0.6, bias.kappa=1,
                            y_pred=y, .RNG.name="base::Wichmann-Hill", .RNG.seed=1234))

inits3 <- dump.format(list( alpha.mu=alpha.mu3, alpha.pr=alpha.si3,
                            b.mu=1.3, b.pr=0.05, bias.mu=0.4, bias.kappa=1,
                            y_pred=y, .RNG.name="base::Mersenne-Twister", .RNG.seed=6666 ))

# parameters to monitor
# monitor = c(
#   "b.mu", "alpha.mu", "bias.mu", "decay.alpha", "decay.beta", "b.p", "alpha.p", "bias.p", "decay.p", "deviance" )
monitor = c(
  "b.mu", "alpha.mu", "bias.mu", "b.p", "alpha.p", "bias.p", "decay.p", "deviance" )
