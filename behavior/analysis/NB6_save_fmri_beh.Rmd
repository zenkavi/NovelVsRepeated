...
data_yn_clean_fmri_preproc %>%
  ...
  ungroup() %>%
  select(reward_dmn, valueRef_dmn, valueStim_dmn, valueChosen_dmn, valueUnchosen_dmn, valueChosenPlusUnchosen_dmn, valueChosenMinusUnchosen_dmn, valueO_dmn, valueS_dmn, valueF_dmn) %>%
  gather(key, value) %>%
  group_by(key) %>%
  summarise(mean_value = round(mean(value), 5))
```
