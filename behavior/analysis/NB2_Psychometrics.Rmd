...
Fixation preprocessing checks

```{r}
names(data_bc_fix)
with(data_bc_fix, table(subnum, day, leftFix))
```
...
data_bc_fix %>%
  filter(firstFix  == 1) %>%
  mutate(fix_on_better_first = ifelse(possiblePayoffleft>possiblePayoffright & leftFix == 1, 1, ifelse(possiblePayoffleft < possiblePayoffright & leftFix == 0, 1, 0))) %>%
  group_by(subnum, day, type) %>%
  summarise(.groups="keep",
            mean_fix_on_better_first = mean(fix_on_better_first),
            sem_fix_on_better_first = sd(fix_on_better_first)/n()) %>%
  ggplot(aes(day, mean_fix_on_better_first, color=type))+
  geom_point()+
  geom_errorbar(aes(ymin = mean_fix_on_better_first - sem_fix_on_better_first, ymax = mean_fix_on_better_first + sem_fix_on_better_first), width = 0)+
  geom_line()+
  facet_wrap(~subnum)+
  scale_color_brewer(palette = "Dark2")+
  labs(x = "Day", y = "p(First fix on better option)", title="BC Task", color="")+
  theme(panel.grid = element_blank(),
        legend.position = "bottom",
        legend.box.margin=margin(t = -10))+
...
```{r}
data_bc_fix %>%
  mutate(fix_on_better = ifelse(possiblePayoffleft>possiblePayoffright & leftFix == 1, "better_fix", ifelse(possiblePayoffleft < possiblePayoffright & leftFix == 0, "better_fix", "worse_fix"))) %>%
  group_by(subnum, day, fix_on_better, type, trialNum) %>%
  summarise(.groups="keep",
            total_fix_duration = sum(fixDuration)) %>%
  spread(fix_on_better, total_fix_duration) %>%
  mutate(better_fix = ifelse(is.na(better_fix), 0, better_fix),
         worse_fix = ifelse(is.na(worse_fix), 0, worse_fix),
         rel_dur_on_better = better_fix/(better_fix+worse_fix)) %>%
  ungroup() %>%
  group_by(subnum, day, type) %>%
  summarise(.groups = "keep",
            mean_rel_dur_on_better = mean(rel_dur_on_better),
            sem_rel_dur_on_better = sd(rel_dur_on_better)/n()) %>%
  ggplot(aes(day, mean_rel_dur_on_better, color=type))+
  geom_point()+
  geom_errorbar(aes(ymin = mean_rel_dur_on_better - sem_rel_dur_on_better, ymax = mean_rel_dur_on_better + sem_rel_dur_on_better), width = 0)+
  geom_line()+
  facet_wrap(~subnum)+
  scale_color_brewer(palette = "Dark2")+
  labs(x = "Day", y = "Relative fixation duration on better option", title="BC Task", color="")+
  theme(panel.grid = element_blank(),
        legend.position = "bottom",
        legend.box.margin=margin(t = -10))+
  scale_x_continuous(breaks = c(3, 7, 11))
...
```{r}
data_bc_fix %>%
  mutate(fix_on_better = ifelse(possiblePayoffleft>possiblePayoffright & leftFix == 1, 1, ifelse(possiblePayoffleft < possiblePayoffright & leftFix == 0, 1, 0))) %>%
  group_by(subnum, day, type, trialNum) %>%
  summarise(.groups="keep",
            total_better_fixes = sum(fix_on_better),
            total_fixes = unique(numTotalFix)) %>%
  mutate(relative_better_fixes = total_better_fixes/total_fixes) %>%
  ungroup() %>%
  group_by(subnum, day, type) %>%
  summarise(.groups = "keep",
            mean_rel_better_fix = mean(relative_better_fixes),
            sem_rel_better_fix = sd(relative_better_fixes)/n()) %>%
  ggplot(aes(day, mean_rel_better_fix, color=type))+
  geom_point()+
  geom_errorbar(aes(ymin = mean_rel_better_fix - sem_rel_better_fix, ymax = mean_rel_better_fix + sem_rel_better_fix), width = 0)+
  geom_line()+
  facet_wrap(~subnum)+
  scale_color_brewer(palette = "Dark2")+
  labs(x = "Day", y = "Relative number of fixations on better option", title="BC Task", color="")+
  theme(panel.grid = element_blank(),
        legend.position = "bottom",
        legend.box.margin=margin(t = -10))+
  scale_x_continuous(breaks = c(3, 7, 11))
